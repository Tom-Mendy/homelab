...
CLUSTER=$(kubectl --kubeconfig "$HOME/.kube/config-homelab" config view -o jsonpath="{.contexts[?(@.name==\"$CTX\")].context.cluster}")
kubectl --kubeconfig "$HOME/.kube/config-homelab" config set-cluster "$CLUSTER" --server=https://192.168.1.11:6443
```
...
```bash
kubectl --kubeconfig "$HOME/.kube/config-homelab" config rename-context kubernetes-admin@kubernetes homelab-admin
